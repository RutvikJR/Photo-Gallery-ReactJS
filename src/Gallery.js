  ...
  return (
    <div>
      <div id="gallery">
        ...
        {[...Array(14)].map((v, i, a) => {
          return (
            <Image key={i} imgclass={"img" + (i + 1)} imgtitle={titles[i]} display={DisplayHandler}></Image>
          );
        })}
      </div>
      {displaystatus &&  <Display displayimgno={display[0]} displaytitle={display[1]} closeDisplay={CloseHandler} Next={NextHandler} Prev={PrevHandler} />}
    </div>
  );
}
